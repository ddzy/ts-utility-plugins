    ...
    (function (Main) {
        ...
        var Fade = /** @class */ (function () {
            ...
            Fade.prototype.handleArrowClick = function () {
                var _this = this;
                var duringTime = yyg_settings.duringTime, easing = yyg_settings.easing, delayTime = yyg_settings.delayTime;
                ...
                var oDotsItem = this.oDotsItem;
                // Right arrow
                oNextArrow && oNextArrow.addEventListener('click', function () {
                    clearInterval(_this.timer);
                    _this.count++;
                    oContentItem.forEach(function (item, index) {
                        if (index === _this.count) {
                ...
                // Left arrow
                oPrevArrow && oPrevArrow.addEventListener('click', function () {
                    clearInterval(_this.timer);
                    _this.count--;
                    oContentItem.forEach(function (item, index) {
                        if (index === _this.count) {
                ...
                oContentItem.forEach(function (item) {
                    item.addEventListener('transitionend', function () {
                        if (_this.count > oContentItemLength - 1) {
                            _this.count = -1;
                            // dot栏样式
                            oDotsItem.forEach(function (ite) {
                                Utils.removeClass(ite, 'yyg-dot-item-active');
                            });
                        }
                        else if (_this.count < 0) {
                            _this.count = oContentItemLength - 1;
                            // dot栏样式
                            oDotsItem.forEach(function (ite) {
                                Utils.removeClass(ite, 'yyg-dot-item-active');
                            });
                            Utils.addClass(oDotsItem[oContentItemLength - 1], 'yyg-dot-item-active');
                        }
                    }, false);
                });
            };
